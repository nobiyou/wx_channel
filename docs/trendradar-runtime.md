...
`grant.json`, the reviewed ltaoo and batch executables, and the active Clash
core/configuration.
...
runtime paths, and the SHA-256 hashes of both executables. The provider deletes
that grant before changing the certificate store or Clash. There is no
...
- A Clash-compatible core whose active YAML has `proxies:`, `rules:`, and a
...
- Permission for the current user to add and remove a certificate in
  `Cert:\CurrentUser\Root` and to replace the selected Clash YAML.
- API/proxy ports (2022/2023 by default) free before the run.

Clash remains running. The provider inserts one marked loopback HTTP proxy and
ordered process rules, validates the candidate with the configured Clash core,
...
3. restore the byte-for-byte Clash backup when no external edit occurred, or
...
request, certificate, private key, Clash configuration, controller secret,
