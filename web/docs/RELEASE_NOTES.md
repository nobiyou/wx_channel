...
- **启动说明修正**：启动界面不再显示 v5.7.0 的评论导出要点，改为完整展示当前 Insight Edge 融合说明。
- **版本同步保护**：新增启动要点版本号、固定五项检查及旧文案残留检查，版本更新遗漏会由测试直接发现。
- **本地日期统计**：修复 UTC+8 凌晨时段今日下载统计和趋势图可能少计的问题。
- **运行行为不变**：继续沿用 v5.7.2 的 Insight Edge 默认链路、协议边界和本地监控行为。
