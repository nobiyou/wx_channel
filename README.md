...
`wx_channel` 通过 Insight 内置的边缘网关进行远程设备管理、数据采集和任务协同：

- Cloud 版通过 `cloud_hub_url` 连接 Insight 的 `/ws/client`
- 本地默认地址为 `ws://127.0.0.1:18081/ws/client`，`config.yaml` 中的显式配置优先
- Insight API 与运营台分别使用 `18081` 和 `5176` 端口
- 独立 `wx_channel_hub` 已停止使用，不再作为运行依赖
...
本次版本聚焦长时间运行稳定性、长标题文件名和版本化页面生命周期：

- **长时间运行保活**：增加页面应用层健康探测、心跳确认和断线重连，识别并恢复协议在线但页面失活的情况。
- **自动打开策略**：新增 `auto_open_channels`，标准版和 Radar 版默认关闭，Cloud 版默认开启。
- **长标题命名**：移除固定 50 字符限制，按完整下载路径动态计算 Windows UTF-16 预算，保留视频 ID、画质和扩展名。
- **下载链路统一**：批量、队列、分片和页面直连下载统一命名与重名规则，数据库保留完整原始标题。
- **公众号请求稳定性**：推送列表读取前等待凭证刷新，降低首次 `401 /api/mp/msg/list` 失败。
...
| 2026-05-27 | Solo* | ￥6.66   | 666，好用，强！！！ |
| 2026-05-28 | 匿名 | ￥1.00   | 工具好用 |
| 2026-06-12 | 匿名 | ￥5.00   | 太强了，请大佬喝杯茶 |
| 2026-06-13 | Har* | ￥5.00   | 感谢 |
| 2026-06-29 | 表情* | ￥5.00   | 未留言 |
| 2026-07-04 | 馒* | ￥1.00   | 未留言 |
| 2026-07-13 | 匿名 | ￥1.00   | 太好用了 |
| 2026-07-20 | 信用* | ￥10.00   | 先请喝茶再用 |
| 2026-07-29 | 嬴* | ￥20.00   | 有Mac版本吗？--回：没有mac电脑 |
