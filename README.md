...
### 高优先级（稳定性/安全/正确性）
- 认证与来源校验：为所有 `__wx_channels_api/*` 接口增加本地令牌/随机密钥或校验 `127.0.0.1` 与受信 Referer
- 上传完整性与断点续传：
  - 分片 `md5/sha256` 校验；`complete_upload` 校验分片数量、大小、顺序
  - 查询已上传分片列表，实现断点续传与重复上传去重
- 文件名/路径稳健性：
  - `GenerateUniqueFilename` 使用 `os.Stat` 判断存在性，避免 `filepath.EvalSymlinks` 误用
  - `EnsureExtension` 仅在无扩展时才补全，避免强制改后缀
- CSV 可靠性与性能：
  - 启动时一次性构建内存索引 `map[id]struct{}`，写入时 O(1) 去重
  - 约束 `record.ID` 不能为空（为空跳过或生成稳定 ID）
- 错误处理与提示：
  - I/O、空间不足、权限不足、文件占用等错误返回结构化错误码与可操作提示
  - 证书失败提供一键打开保存目录提示与说明链接

### 中优先级（体验/可维护性）
- 并发与节流：分片写入与合并加入限流器；合并使用较大缓冲
- 下载目录策略：支持 UI/配置选择保存根目录、作者子目录开关、文件名模板（时间戳/作者/标题/ID）
- 日志与可观测性：文件滚动日志（大小/日期），关键操作统一结构化日志
- 配置与版本化：支持 `config.json`/环境变量加载，命令行覆盖；版本语义化并在构建时注入
- API 规范化：统一 `{success, data, error, code}`；附最小示例（curl）
- Windows 兼容：路径统一使用 `filepath.*`；控制台 UTF-8 输出优化
- 注入脚本健壮性：DOM/接口变动容错、失败重试与可视化反馈

### 低优先级（功能增强）
- 批量下载：队列与并发控制、失败重试/清单导出、进度持久化
- 导出/记录多格式：支持 JSON/CSV/Markdown，CSV 字段可配置
- 托盘与最小化：Windows 托盘图标、打开下载目录、启动/停止代理、查看日志
- 证书与代理体验：权限不足时引导“以管理员身份重启”；一键打开系统证书管理器/代理设置
...
>>>>>>> dbf3c2c67e6e08ba970f329180c3438b07fe70b6
