...
- ✅ **智能去重**：自动识别已下载视频，避免重复
- ✅ **无感雷达**：(v5.6.1 延续优化) 默认关闭，支持在控制台中即时启停的静默雷达监控
...
本次补丁版本重点修复了新版视频号中“`/pages/home` 但实际渲染为详情页模式”的兼容问题，并继续增强评论采集与分享直达视频下载的稳定性：

- **Home 详情页模式修复**:
  - 兼容 `oid/nid`、`context_id`、`fromSubPage=profile` 等不同直达视频链接
  - 修复分享出去的视频、个人页跳转视频在 `home` 路径下下载按钮灰色不可用的问题
  - 对“URL 是 home，实际页面是详情页”的场景自动切换到 `feed` 逻辑处理
- **评论采集修复**:
  - 修复 `Home` 路径详情页模式点击“采集评论”提示功能未就绪的问题
  - 补齐这类页面的评论采集脚本注入
  - 继续保持新版评论侧栏结构兼容与自动采集能力
- **稳定性延续优化**:
  - 延续 `v5.6.0` 对新版首页、`feed`、`profile`、`赞和收藏`、Hub 与雷达的适配与稳定性增强
