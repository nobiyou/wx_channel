...
本次版本将视频号页面生命周期交给 wx_channel，自动检测、打开并恢复视频号页面：

- **视频号生命周期**：检测 `Weixin.exe`，微信运行时通过 `weixin://dl/channels` 打开视频号页面。
- **页面主动恢复**：使用健康阈值、退避重试和冷却状态恢复失活页面。
- **定向页面命令**：只向匹配的视频号页面发送刷新或导航命令。
- **刷新责任收敛**：移除页面固定 15 分钟自动刷新，保留评论导出刷新锁。
- **Insight Edge 默认链路**：Cloud 版继续连接本地 Insight `18081/ws/client`，现有配置继续有效。
