...
本次版本新增公众号资料库，并继续加固视频号页面与控制台：

- **公众号资料库**：自动捕获已登录微信中的公众号账号，按 `biz` 去重并建立本地文章目录。
- **历史文章管理**：支持最近一页或全部历史同步，分页保存进度，支持中断后继续和重复同步去重。
- **文章归档与指标**：保存 HTML/资源归档及 manifest，追加记录阅读、点赞、评论、转发、收藏和赞赏指标。
- **数据迁移**：支持 JSON/ZIP 导入导出、预检和冲突处理，导出不包含短期凭证。
- **入口和稳定性**：公众号与视频号页面新增管理控制台入口，并修复页面 API 客户端重连边界。
