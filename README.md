...
本次补丁版本聚焦分享链接解析链路，既恢复 `/api/channels/shared_feed/profile`，也补上了 Web 控制台直接导入分享链接的能力：

- **分享详情恢复**：修复 `shared_feed/profile` 分享链接详情接口回归，重新可获取视频信息。
- **控制台直接导入**：批量下载页新增“分享链接下载”，解析后可直接加入下载列表。
- **双通道解析**：支持自动、视频号页面、Cookie/Worker 纯后端三种解析模式。
- **短链回退补齐**：当页面接口未稳定返回 `dynamicExportId` 时，自动回退到短链 ID。
- **后端状态可见**：设置接口透出后端解析配置状态，页面可直接提示是否已配置 Cookie/Worker。
- **版本说明同步**：启动横幅、README、版本文档与 Windows 文件版本信息统一更新到本次发布。
