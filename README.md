...
本次版本完成 Insight Hub 接入，并继续强化评论导出和云端连接稳定性：

- **Insight Hub 集成**：Cloud 版默认连接本地 Insight `18081/ws/client`，支持配置远程 Insight 地址。
- **云端连接稳定**：增强 WebSocket 断线切换、API 超时取消和边缘节点状态同步。
- **评论异步导出**：评论导出改为后台任务，前端通过状态接口轮询，避免长请求触发 `Failed to fetch` 误报。
- **导出进度保护**：支持评论分页上限、`.partial.json` 检查点和中断后的进度保留。
- **结果核对增强**：成功提示展示一级评论、回复和合计条数；雷达版仍需单独版本并默认关闭。
