...
本次版本修复微信 CDN 原始流规则变化导致的“下载成功但不是原片”问题：

- **原始流识别**：只有微信明确提供 `fullUrl` 时才标记原始视频。
- **最高画质回退**：没有原始流时自动选择最高可用规格，当前桌面端优先 `xWT111`。
- **下载链路加固**：保留完整签名 URL，页面直连失败自动回退后端。
- **低码率防误报**：按源文件大小校验，拦截明显低于源文件提示的转码流。
- **模式明确**：下载界面明确区分原始视频和最高可用画质。
