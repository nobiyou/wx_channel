...
本次补丁版本主要聚焦批量下载链路稳定性，重点修复部分视频无法播放以及暂停恢复不真实的问题：

- **下载落盘加固**：下载、解密、校验与最终落盘分阶段完成，异常中断时更易清理和恢复。
- **不可播放问题修复**：降低加密视频在批量下载后出现损坏或无法播放的概率。
- **真实暂停恢复**：批量任务改为真实调用 Gopeed 的 `Pause` / `Continue`，恢复行为更符合预期。
- **并发稳定性提升**：并发下载场景下任务状态与最终文件结果更一致。
- **版本说明同步**：启动横幅、版本文档与 Windows 文件版本信息统一更新到本次发布。
