...
- **雷达下载问题修复**: 完全重构了雷达视频解析逻辑。以前每个视频都需要单独调用 `feed_profile` API 接口获取详情，容易导致超时失败。现在改为像订阅功能一样，直接从 `feed_list` 中精准提取视频 URL、解密密钥、封面、时长等信息，彻底解决了新视频加入队列后卡在等待中的问题。
- **批量下载恢复修复**: 修复了“全部恢复”按钮点击后只恢复已暂停任务，而不处理 `pending` (等待中) 任务的 BUG。现在可一键将所有等待中的任务正确加入后台下载流水线中。
- **下载记录重复问题**: 修复了下载记录表中单个视频出现两条记录的问题（一条 UUID 主键，一条 VideoID 主键）。修改了 `queue_service.go` 的底层防抖与去重逻辑，确保由不同渠道(如前后端状态轮询)标记下载完成的调用能够正确识别现有记录。
